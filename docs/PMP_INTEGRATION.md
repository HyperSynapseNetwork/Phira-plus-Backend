...
PPB 通过 OpenUDS（Unix domain socket）控制 PMP，从不直连 PMP PostgreSQL。数据所有权见 contract §13。

## 连接与认证

- 帧：4 字节 LE 长度前缀 + UTF-8 JSON，最大 16 MiB。
- 认证：`{"type":"authenticate","token":...}` 或 approve 模式 `{"type":"authenticate","client_name":...}` → `auth_pending` → `approve openuds <id>`。
- 认证后建立能力集（版本映射 + capability detection，见 `pmp/capabilities.rs`）。
- 断线自动重连（指数退避 + 抖动，`pmp/openuds/client.rs`）。
...
禁止用 `broadcast.room` 冒充玩家聊天。信封：`{id,type,version,occurred_at,resource:{type,id},data}`。
